 ...
 * sections → Footer. `os` (persisted via useOS) and `query` (session-only)
 * are still owned here and threaded down to every consumer.
 */
function App() {
  const [os, setOs] = useOS();
  const [query, setQuery] = useState("");

  const filteredCategories = useShortcutFilter(categories, query, os);
